...
int main() {
  // Initial Conditions
  vector<Body> B = {Body(100, 0, 0, 0, 0, 0, 0), Body(1, 20, 0, 0, 0, sqrt(5), 0)};
  ...
  file1.open("positions2.txt");
  energyFile.open("energy_2.txt");

  for (double t = 0; t <= T; t += h) {
    B = gravity_solver.rk4(B, 2, h);
    double totalEnergy = gravity_solver.totalEnergy(B, 2);
